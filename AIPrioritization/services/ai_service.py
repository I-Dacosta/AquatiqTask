...
# Initialize OpenAI client (v1.0+ API) - only if API key is provided
client = None
...
class AIModel:
    ...
    def calculate_priority_metrics(self, task: TaskRequest) -> PriorityMetrics:
        ...
        business_value = task.business_value or 5
        affected_users = task.affected_users_count or 1
        business_impact_score = min(10.0, business_value + math.log10(affected_users))
        
        # Risk score (direct from task + category multiplier)
        risk_level = task.risk_level or 5
        risk_score = min(10.0, risk_level * self.category_urgency.get(task.category, 1.0))
        ...
        effort_hours = task.estimated_effort_hours or 5.0
        effort_complexity_score = max(1.0, 10.0 - (effort_hours * 0.5))
    ...
    async def generate_risk_assessment(self, task: TaskRequest, metrics: PriorityMetrics) -> str:
        ...
        try:
            ...
            Title: {task.title}
            Description: {task.description}
            Risk Level: {task.risk_level or 5}/10
            Business Value: {task.business_value or 5}/10
            Affected Users: {task.affected_users_count or 1}
            Category: {task.category}
...
async def get_ai_priority(task: TaskRequest) -> AIPriorityResult:
    ...
    try:
        ...
        # Update task with dynamically calculated metrics (only if not already set)
        if task.business_value is None:
            task.business_value = local_analysis['business_value']
        if task.risk_level is None:
        ...
        if task.workaround_available is None:
            task.workaround_available = local_analysis['workaround_available']
        if task.affected_users_count is None:
